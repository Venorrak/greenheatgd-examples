...
var cursors : Dictionary[String, Cursor] = {} # memory of all the twitchIds pointing to their cursor

func _ready() -> void:
	# connect all GreenHeat signals
...
func _click_received(packet: Dictionary) -> void:
	var cur = cursors.get(packet['id']) # get cursor
	if not cur: _create_cursor(packet); return # create cursor if it doesn't exist
	cur.targetPosition = _get_position_from_packet(packet) # update cursor state
	cur.lastPing = 0
...
func _create_cursor(packet: Dictionary) -> void:
	var isGrabbing : bool = packet["type"] == "drag" or packet["type"] == "click" # check the state for sprite
	var _position : Vector2 = _get_position_from_packet(packet)
	var newCursor : Cursor = cursorScene.instantiate()
	newCursor.userId = packet["id"] # Set the different params of the cursor
	...
	newCursor.deleteSelf.connect(_delete_cursor) # connect signal for self destruct
	cursors.set(packet["id"], newCursor) # put the cursor in the memory
	add_child(newCursor) # add cursor to the tree

func _get_position_from_packet(packet: Dictionary) -> Vector2:
	# maps the position of the mouse to the size of the "Game"
	return Vector2(packet["x"], packet["y"]) * get_tree().root.get_visible_rect().size 

func _delete_cursor(cursor : Cursor) -> void: # called by cursor when too old
	cursor.queue_free()
